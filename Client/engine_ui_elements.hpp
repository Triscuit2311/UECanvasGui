...
struct Control
{
	...
	{
		return {ui_style::control_dimensions.w, ui_style::control_dimensions.h};
	}
...
public:
	...
	Window() : next_control_position_{ui_style::margins.lr, ui_style::margins.tb + ui_style::control_dimensions.h}
	{
	}
	...
	void add_control(Args&&... args)
	{
		...
		size.Y =
			((controls_.size() + 1) * ui_style::control_dimensions.h) +
			((controls_.size()) * ui_style::spacing) +
			ui_style::margins.tb;

		size.X = ui_style::control_dimensions.w + (ui_style::margins.lr * 2);
	}

	bool is_hovered(const SDK::FVector2D &cursor_pos) const;
	void render(const SDK::FVector2D& cursor_pos);
